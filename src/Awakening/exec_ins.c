...
** Last update Sun May 17 15:01:36 2015 guigui
*/
...
int	exec_simpleins(t_dtf *map, char *ins)
{
  ...
  if (!my_strcmp(ins, STRATG))
    ...
    return (map->pass(map));
  else if (!my_strcmp(ins, SHOWMP))
    return (map->show_map(map));
...
int	exec_complexins(t_dtf *map, char **tab)
{
  ...
  else if (count_els((void **)tab) == 3 && !my_strcmp(tab[1], ATTACK))
    return (map->attack(map, my_atoi(tab[0]), my_atoi(tab[2])));
  else if (count_els((void **)tab) == 2 && !my_strcmp(tab[0], ISUSED))
    return (map->is_unitused(map, my_atoi(tab[1])));
  else
    {
      my_putstr(ERRCMD);
      return (dbgerr("Error: exec_simpleins: Unknow command"));

/*
** get_next_line.c for gnl in /home/guigui/synaptique-pirou_g/gnl
** 
** Made by guigui
** Login   <guigui@epitech.net>
** 
** Started on  Tue Feb  3 16:03:13 2015 guigui
** Last update Fri May 15 21:42:25 2015 
*/
...
char   	*extract_line(t_gnl *file, char *str, int len)
{
  ...
  if (file == NULL || str == NULL || (out = gbgc_malloc(NULL, len + 1)) == NULL)
    return (NULL);
  i = -1;
  while (++i < len && str[i] != '\n' && str[i] != '\0')
    out[i] = str[i];
...
char	*loop_gnl(t_gnl *file, char *line, char *tmp, int len)
{
  ...
  while ((!is_line(line) && file->status != MEOF))
    {
      if ((i = read(file->fd, file->buff, BUFFER_SIZE)) < 0 ||
...
char		*get_next_line(int fd)
{
  ...
  if (fle == NULL || (fle->fd != fd && fd >= 0))
	...
	return (NULL);
      ...
      fle->buff[0] = '\0';
      fle->status = NEOF;
      fle->buff[read(fle->fd, fle->buff, BUFFER_SIZE)] = '\0';
    }
